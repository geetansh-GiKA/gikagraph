...
//     const width = Number(bw) || 1
//     const offset = Number(v) || 0
//     // move between -width and 0, then wrap – smoother loop
//     return `${wrap(-width, 0, offset)}px`
//   })
...
//     >
//       <motion.div
//         className="inline-flex transform-gpu items-center will-change-transform select-none gap-8"
//         style={{ x }}
//       >
...
//             aria-hidden={i !== 0}
//             className="inline-flex shrink-0 items-center pr-24"
//           >
//             {children}
//           </div>
//         ))}
...
function ScrollVelocityRowImpl({
  ...
  // 🔑 Clean wrapping of x between -width and 0
  const x = useTransform([baseX, unitWidth], ([v, bw]) => {
    const width = Number(bw) || 1
    const offset = Number(v) || 0
    // move in [-width, 0), wrapping smoothly
    return `${wrap(-width, 0, offset)}px`
  })
  ...
  useAnimationFrame((_, delta) => {
    ...
    // 🔒 direction is fixed by prop; no flipping based on scroll sign
    const moveBy =
      baseDirectionRef.current * pixelsPerSecond * speedMultiplier * dt
      ...
      >
        {Array.from({ length: numCopies }).map((_, i) => (
          <div
            ...
            ref={i === 0 ? blockRef : null}
            aria-hidden={i !== 0}
            // 👇 spacing lives inside the measured block so loop distance matches visuals
            className="inline-flex shrink-0 items-center pr-24"
          >
            {children}
          </div>
