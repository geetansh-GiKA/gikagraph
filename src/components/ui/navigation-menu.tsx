...
function NavigationMenuTrigger({
	...
	return (
		<NavigationMenuPrimitive.Trigger
			...
			className={cn(
				'group border border-transparent hover:border-foreground hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground data-[state=open]:hover:bg-accent data-[state=open]:text-accent-foreground data-[state=open]:focus:bg-accent data-[state=open]:bg-accent/50 focus-visible:ring-ring/50 inline-flex w-max items-center justify-center rounded-md px-4 py-1 text-sm font-medium transition-[color,box-shadow,border-color] outline-none focus-visible:ring-[3px] focus-visible:outline-1 disabled:pointer-events-none disabled:opacity-50',
				className,
			)}
			{...props}
		>
...
function NavGridCard({
	...
	return (
		<GridCard className={className} {...props}>
			<NavigationMenuPrimitive.Link asChild>
				...
				>
					<div className="relative flex items-center gap-2">
						{link.icon && (
							<link.icon className="text-foreground/80 size-5" />
						)}
			...
			{subLinks && subLinks.length > 0 && (
				<div className="relative mt-2 grid grid-cols-2 gap-2 ">
					{subLinks.map((subLink) => {
						const subIsExternal = subLink.href.startsWith('http');
						return (
							<NavigationMenuPrimitive.Link asChild key={subLink.href}>
								<a
									href={subLink.href}
									target={subIsExternal ? '_blank' : undefined}
									rel={subIsExternal ? 'noopener noreferrer' : undefined}
									className="hover:bg-accent hover:text-accent-foreground flex items-center gap-2 rounded-sm border px-2 py-1.5 text-xs transition-colors"
								>
									{subLink.icon && <subLink.icon className="size-3.5" />}
									{subLink.title}
								</a>
							</NavigationMenuPrimitive.Link>
						);
...
function NavItemMobile({
	...
	return (
		...
		>
			<div
				className={cn(
					'bg-muted/20 flex size-10 shrink-0 items-center justify-center rounded-lg border',
				)}
			>
